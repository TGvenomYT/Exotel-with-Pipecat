...
# # Import bot and shared store
# from bot import bot, booking_info_store
...
#         logger.error("Missing Exotel credentials!")
...
#     logger.info(f"Initiating Exotel call: From={from_number}, To={to_number}")
...
#             logger.info(f"Raw Exotel response: {result_text}")
#             if response.status != 200:
#                 logger.error(f"Exotel API error ({response.status}): {result_text}")
...
#             if call_sid == "unknown":
#                logger.warning("Could not extract <Sid> from Exotel response text!")

#             logger.info(f"Exotel call successfully initiated. CallSid: {call_sid}")
#             return {"status": "call_initiated", "call_sid": call_sid}
#     except Exception as e:
#         logger.error(f"Network error calling Exotel: {e}")
#         raise
...
#     logger.info("Received POST /start request")
#     try:
#         data = await request.json()
#         if not data.get("dialout_settings"):
#             raise HTTPException(status_code=400, detail="Missing 'dialout_settings'")

#         phone_number = str(data["dialout_settings"].get("phone_number"))
...
#         if call_sid != "unknown":
#             logger.info(f"Storing booking info for {call_sid}: {booking_info}")
#             booking_info_store[call_sid] = booking_info
#             booking_info_store[f"call_{call_sid}"] = booking_info
#             logger.info(f"Stored booking info for {call_sid}")
...
# Added imports required by the code
from loguru import logger
from bot import bot, booking_info_store
